import { Dua } from '../types/islamic';

export const dailyDuas: Dua[] = [
  {
    title: "Doa Bangun Tidur",
    arabic: "الْحَمْدُ لِلَّهِ الَّذِي أَحْيَانَا بَعْدَ مَا أَمَاتَنَا وَإِلَيْهِ النُّشُورُ",
    translation: "Segala puji bagi Allah yang telah menghidupkan kami setelah mematikan kami dan kepada-Nya kebangkitan.",
    transliteration: "Alhamdulillahilladzi ahyaana ba'da maa amaatanaa wa ilaihin nushur"
  },
  {
    title: "Doa Sebelum Makan",
    arabic: "بِسْمِ اللَّهِ وَعَلَى بَرَكَةِ اللَّهِ",
    translation: "Dengan nama Allah dan atas berkah Allah.",
    transliteration: "Bismillahi wa 'alaa barakatillahi"
  },
  {
    title: "Doa Sesudah Makan",
    arabic: "الْحَمْدُ لِلَّهِ الَّذِي أَطْعَمَنَا وَسَقَانَا وَجَعَلَنَا مُسْلِمِينَ",
    translation: "Segala puji bagi Allah yang telah memberi makan dan minum kepada kami dan menjadikan kami muslim.",
    transliteration: "Alhamdulillahilladzi ath'amanaa wa saqaanaa wa ja'alanaa muslimiin"
  },
  {
    title: "Doa Keluar Rumah",
    arabic: "بِسْمِ اللَّهِ تَوَكَّلْتُ عَلَى اللَّهِ لاَ حَوْلَ وَلاَ قُوَّةَ إِلاَّ بِاللَّهِ",
    translation: "Dengan nama Allah, aku bertawakal kepada Allah. Tidak ada daya dan kekuatan kecuali dengan Allah.",
    transliteration: "Bismillahi tawakkaltu 'alallahi laa hawla wa laa quwwata illaa billahi"
  },
  {
    title: "Doa Masuk Rumah",
    arabic: "اللَّهُمَّ إِنِّي أَسْأَلُكَ خَيْرَ الْمَوْلِجِ وَخَيْرَ الْمَخْرَجِ",
    translation: "Ya Allah, sesungguhnya aku memohon kepada-Mu kebaikan tempat masuk dan kebaikan tempat keluar.",
    transliteration: "Allahumma inni as'aluka khairal mauliji wa khairal makhraji"
  },
  {
    title: "Doa Ketika Hujan",
    arabic: "اللَّهُمَّ صَيِّبًا نَافِعًا",
    translation: "Ya Allah, jadikanlah hujan yang bermanfaat.",
    transliteration: "Allahumma shayyiban naafi'aan"
  }
];